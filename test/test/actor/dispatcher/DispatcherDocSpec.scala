...
import test.actor.doc.MyUnboundedMessageQueueSemantics
...
object DispatcherDocSpec {
  val javaConfig = """
    ...
    prio-mailbox {
      mailbox-type = "test.actor.doc$MyPrioMailbox"
    ...
    custom-dispatcher {
      mailbox-requirement =
      "test.actor.doc.MyUnboundedMessageQueueSemantics"
    }
  ...
  val config = """
    ...
    prio-dispatcher {
      mailbox-type = "test.actor.dispatcher.DispatcherDocSpec$MyPrioMailbox"
      //Other dispatcher configuration goes here
    }
    //#prio-dispatcher-config
    ...
    custom-dispatcher-mailbox {
      mailbox-type = "test.actor.doc.MyUnboundedMailbox"
    }
    //#custom-mailbox-config
...
class DispatcherDocSpec1 extends AkkaSpec(DispatcherDocSpec.config) {
